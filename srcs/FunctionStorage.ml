...
let to_string (func:string) (var:string) =
  let e = Hashtbl.find fun_hashtbl (func, var) in
  Printf.sprintf "(%s, %s, %s)" func var (Solver.res_to_string e)
...
let print_state () =
  ...
  if Hashtbl.length fun_hashtbl = 0 then
    ...
    let print_function ((func:string), (x:string)) (value:Var.VariableWithComplex.t) =
